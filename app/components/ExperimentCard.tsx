...
const cardClass =
  "group flex flex-col overflow-hidden border border-border bg-surface transition-colors hover:border-muted md:flex-row";

export function ExperimentCard({ experiment, onOpenModal }: ExperimentCardProps) {
  ...
  if (onOpenModal) {
    return (
      <button type="button" onClick={onOpenModal} className={`${cardClass} w-full text-left cursor-pointer`}>
        {thumb}
        {body}
      </button>
    );
